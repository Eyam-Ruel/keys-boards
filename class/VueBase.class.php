    ...
    public function afficher() {
        ...
        <head>
            ...
            <style>
                ...
                .btn-global-post {
                    background-color: #810F29; color: white; border: none; 
                    border-radius: 25px; padding: 12px; width: 100%; 
                    margin-top: 20px; font-weight: bold; cursor: pointer; 
                    display: flex; align-items: center; justify-content: center; gap: 10px;
                    transition: opacity 0.2s;
                }
                .btn-global-post:hover { opacity: 0.9; }

                /* Music Boards Styles */
                .music-boards-divider { height: 1px; background: #E9DDE1; margin: 26px 0 22px 0; }
                .music-boards-title { display: flex; align-items: center; gap: 12px; margin-bottom: 18px; color: #8D0F2D; font-weight: 700; font-size: 14px; text-transform: uppercase; }
                .music-boards-list { display: flex; flex-direction: column; gap: 8px; }
                .music-board-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 6px; border-radius: 10px; text-decoration: none; color: #2F0610; }
                .music-board-item:hover { background: rgba(141, 15, 45, 0.06); }
            </style>
        </head>
        <body>
                ...
                <div class="topbar-icons">
                    ...
                    <div class="lang-dropdown-container">
                        ...
                        <div class="lang-menu" id="langMenu">
                            <a href="?action=' . $this->actionActive . '&lang=en" class="lang-option">🇬🇧 English</a>
                            <a href="?action=' . $this->actionActive . '&lang=fr" class="lang-option">🇫🇷 Français</a>
                            <a href="?action=' . $this->actionActive . '&lang=sq" class="lang-option">🇦🇱 Shqip</a>
                            <a href="?action=' . $this->actionActive . '&lang=vi" class="lang-option">🇻🇳 Tiếng Việt</a>
                        </div>
                    </div>
                </div>
            ...
            echo '<aside class="sidebar">
                <nav class="nav-menu">
                    <a href="index.php?action=feed" class="nav-link ' . ($this->actionActive == 'feed' ? 'active' : '') . '">
                        ...
                        <span class="nav-label">' . ($trad['side_home'] ?? 'Home') . '</span>
                    </a>
                    <a href="index.php?action=explore" class="nav-link ' . ($this->actionActive == 'explore' ? 'active' : '') . '">
                        <img src="./img/icons/Explore.png" class="nav-img">
                        <span class="nav-label">' . ($trad['side_explore'] ?? 'Explore') . '</span>
                    </a>
                    <a href="index.php?action=notif" class="nav-link ' . ($this->actionActive == 'notif' ? 'active' : '') . '">
                        <img src="./img/icons/Notifications.png" class="nav-img">
                        <span class="nav-label">' . ($trad['side_notifications'] ?? 'Notifications') . '</span>
                    </a>
                    <a href="index.php?action=follow" class="nav-link ' . ($this->actionActive == 'follow' ? 'active' : '') . '">
                        <img src="./img/icons/Community.png" class="nav-img">
                        <span class="nav-label">' . ($trad['side_follow'] ?? 'Follow') . '</span>
                    </a>
                    <a href="index.php?action=messages" class="nav-link ' . ($this->actionActive == 'messages' ? 'active' : '') . '">
                        <img src="./img/icons/Messages.png" class="nav-img">
                        <span class="nav-label">' . ($trad['side_messages'] ?? 'Messages') . '</span>
                    </a>
                    <a href="index.php?action=agenda" class="nav-link ' . ($this->actionActive == 'agenda' ? 'active' : '') . '">
                        <img src="./img/icons/Agenda.png" class="nav-img">
                        <span class="nav-label">' . ($trad['side_agenda'] ?? 'Agenda') . '</span>
                    </a>
                    <a href="index.php?action=profile" class="nav-link ' . ($this->actionActive == 'profile' ? 'active' : '') . '">
                        ...
                        <span class="nav-label">' . ($trad['side_profile'] ?? 'Profile') . '</span>
                    </a>

                    <button id="openPostModalGlobal" class="btn-global-post">
                        <span style="font-size: 20px;">+</span> ' . ($trad['btn_post'] ?? 'Post') . '
                    </button>
                </nav>
                <div class="music-boards-divider"></div>
                <div class="music-boards-title"><span>#</span><span>MUSIC BOARDS</span></div>
                <div class="music-boards-list">
                    <a href="#" class="music-board-item"><span class="music-board-left"><span>#Jazz</span></span><span class="music-board-count">12.5k</span></a>
                    <a href="#" class="music-board-item"><span class="music-board-left"><span>#Vinyl</span></span><span class="music-board-count">15.3k</span></a>
                </div>
            </aside>';
        }
        ...
        <div id="postModal" class="create-post-modal hide">
            <div class="create-post-card">
                ...
                <form id="createPostForm" enctype="multipart/form-data">
                    <textarea name="content" id="postContent" placeholder="' . ($trad['pulse_new_post_placeholder'] ?? 'Quoi de neuf ?') . '" required></textarea>
                    <input type="file" name="media" id="postMedia" style="display:none;" accept="image/*" />
                    <div class="create-post-form-footer">
                        <div class="create-post-icons" style="cursor:pointer;"><img src="img/icons/Explore.png" width="24"></div>
