...
class VuePrivacy {
    ...
    public function afficher() {
        ...
        <head>
            ...
            <title><?= $trad['privacy_title'] ?? 'Privacy Policy - Keys & Boards' ?></title>
            ...
            <style>
                ...
                .signout-btn { border: none; background: transparent; font-size: 13px; font-weight: 500; color: var(--text-dark); cursor: pointer; font-family: "DM Sans", sans-serif; }
                ...
                .legal--header-content { max-width: 900px; margin: 0 auto; }
                .legal--back-link { color: white; text-decoration: none; font-size: 14px; display: flex; align-items: center; justify-content: center; gap: 8px; margin-bottom: 25px; opacity: 0.9; }
                .legal--header h1 { font-family: "Jost", sans-serif; font-size: 36px; margin: 0 0 10px 0; }
                .legal--header p { font-size: 18px; opacity: 0.9; margin: 0; }

                /* CONTENU */
                .legal--main { display: flex; justify-content: center; padding: 40px 20px 80px 20px; }
                .legal-container { background-color: white; width: 100%; max-width: 900px; border-radius: 12px; padding: 50px; box-shadow: 0 4px 20px rgba(0,0,0,0.03); }
                
                .legal-main-title { color: var(--main-burgundy); font-size: 24px; font-weight: bold; margin-bottom: 25px; border-bottom: 1px solid rgba(129,15,41, 0.1); padding-bottom: 10px; }
                .legal-section-title { color: var(--main-burgundy); font-size: 19px; font-weight: bold; margin: 30px 0 12px 0; }
                .legal-block { background: #F8F9FA; border-radius: 12px; padding: 25px; margin-bottom: 15px; line-height: 1.6; color: #444; }
                strong { color: var(--text-dark); }
                
                .warning-box { 
                    border-left: 4px solid var(--main-burgundy); 
                    background: #FFF8F9; 
                    padding: 20px; 
                    margin-top: 25px; 
                    border-radius: 8px; 
                }
                .warning-box strong { color: var(--main-burgundy); display: block; margin-bottom: 5px; }
                .last-updated { text-align: center; color: #888; font-size: 12px; margin-top: 40px; }
        ...
        <body>
                    ...
                    <div class="topbar-icons">
                        <div class="lang-dropdown-container">
                            ...
                            <div class="lang-menu" id="langMenu">
                                <a href="?action=privacy&lang=en" class="lang-option">🇬🇧 English</a>
                                <a href="?action=privacy&lang=fr" class="lang-option">🇫🇷 Français</a>
                                <a href="?action=privacy&lang=sq" class="lang-option">🇦🇱 Shqip</a>
                                <a href="?action=privacy&lang=vi" class="lang-option">🇻🇳 Tiếng Việt</a>
                            </div>
                        </div>
                    </div>
            ...
            <header class="legal--header">
                <div class="legal--header-content">
                    ...
                    <h1><?= $trad['footer_link_privacy'] ?? 'Privacy Policy' ?></h1>
            ...
            <main class="legal--main">
                <div class="legal-container">
                    <h1 class="legal-main-title"><?= $trad['privacy_main_h1'] ?? '3. Privacy Policy & Cookie Management' ?></h1>

                    <h2 class="legal-section-title"><?= $trad['privacy_s1_title'] ?? '3.1. Interpretation and Definitions' ?></h2>
                    <div class="legal-block">
                        <?= $trad['privacy_s1_intro'] ?? 'For the purposes of this Privacy Policy:' ?><br><br>
                        <?= $trad['privacy_def_account'] ?? '<strong>Account</strong> means a unique account created for You to access our Service.' ?><br>
                        <?= $trad['privacy_def_company'] ?? '<strong>Company</strong> refers to Keys & Boards, 1 Rue de Chablis, 93000 Bobigny.' ?><br>
                        <?= $trad['privacy_def_country'] ?? '<strong>Country</strong> refers to: France.' ?><br>
                        <?= $trad['privacy_def_device'] ?? '<strong>Device</strong> means any device that can access the Service such as a computer, a cellphone or a digital tablet.' ?><br>
                        <?= $trad['privacy_def_service'] ?? '<strong>Service</strong> refers to the Keys & Boards Website.' ?>
                    </div>

                    <h2 class="legal-section-title"><?= $trad['privacy_s2_title'] ?? '3.2. GDPR & Database Compliance' ?></h2>
                    <div class="legal-block">
                        <?= $trad['privacy_s2_text'] ?? 'This private network strictly complies with the General Data Protection Regulation (GDPR). The collection and storage of data conform the national and international regulations concerning databases.' ?>
                    </div>

                    <h2 class="legal-section-title"><?= $trad['privacy_s3_title'] ?? '3.3. Data Collection and Usage' ?></h2>
                    <div class="legal-block">
                        <?= $trad['privacy_s3_intro'] ?? 'We collect personal data to provide and improve the Service. This includes:' ?><br><br>
                        <strong><?= $trad['privacy_data_profile_title'] ?? 'Profile Data:' ?></strong> <?= $trad['privacy_data_profile_text'] ?? 'Spoken languages, geographical location, and shared skills/experiences.' ?><br><br>
                        <strong><?= $trad['privacy_data_comm_title'] ?? 'Communication Data:' ?></strong> <?= $trad['privacy_data_comm_text'] ?? 'To protect your privacy, our internal messaging system allows users to exchange messages via the platform without a need to disclose their e-mail addresses in the first place.' ?><br><br>
                        <strong><?= $trad['privacy_data_media_title'] ?? 'Image and Video Data:' ?></strong> <?= $trad['privacy_data_media_text'] ?? 'Any use of a person\'s image on publicly accessible areas of the site requires prior written consent from the people featured.' ?>
                    </div>

                    <h2 class="legal-section-title"><?= $trad['privacy_s4_title'] ?? '3.4. User Rights (GDPR)' ?></h2>
                    <div class="legal-block">
                        <?= $trad['privacy_s4_text'] ?? 'Under the General Data Protection Regulation, you have the right to access, rectify, port, and erase your personal data at any time. You also have the right to object to or restrict the processing of your data. To exercise these rights, users can contact the Publisher directly via the internal messaging system or by email at azad.oztopal.pro@gmail.com.' ?>
                    </div>

                    <h2 class="legal-section-title"><?= $trad['privacy_s5_title'] ?? '3.5. Cookie Policy & Retention Period' ?></h2>
                    <div class="legal-block">
                        <?= $trad['privacy_s5_intro'] ?? 'When you visit Keys & Boards, cookies are placed on your device:' ?><br><br>
                        <strong><?= $trad['privacy_cookie_ess_title'] ?? 'Essential Cookies:' ?></strong> <?= $trad['privacy_cookie_ess_text'] ?? 'Required to provide you with services available through the Website and enable core functionalities like secure login.' ?><br><br>
                        <strong><?= $trad['privacy_cookie_func_title'] ?? 'Functional Cookies:' ?></strong> <?= $trad['privacy_cookie_func_text'] ?? 'Allow the Website to remember choices you make (like your language preference).' ?><br><br>
                        <?= $trad['privacy_cookie_retention'] ?? 'In accordance with current regulations, cookies requiring your consent are kept for a maximum of 13 months from their deposit on your device. Associated analytics data is kept for a maximum of 24 months.' ?>
                    </div>

                    <h2 class="legal-section-title"><?= $trad['privacy_s6_title'] ?? '3.6. How to manage cookies' ?></h2>
                    <div class="legal-block">
                        <?= $trad['privacy_s6_intro'] ?? 'You can set your browser to block cookies:' ?><br><br>
                        <strong>Google Chrome:</strong> Menu > Settings > Privacy and security > Cookies and other site data.<br>
                        <strong>Mozilla Firefox:</strong> Menu > Settings > Privacy & Security > Cookies and Site Data.
                    </div>

                    <div class="warning-box">
                        <strong><?= $trad['privacy_warning_title'] ?? 'Warning' ?></strong>
                        <?= $trad['privacy_warning_text'] ?? 'Blocking all cookies may restrict your access to certain features of the Keys & Boards site.' ?>
                    </div>

                    <div class="last-updated"><?= $trad['privacy_last_update'] ?? 'Last updated: March 30, 2026' ?></div>
                </div>
            </main>
